<Collapsible bind:open={state} className="w-full space-y-1">
	...
	<div slot="content">
		...
		{#each (status.query_summaries ? status.query_summaries : (status.query || [])) as query}
			<div
				class="text-sm border border-gray-300/30 dark:border-gray-700/50 rounded-xl mb-4"
			>
				<a
				...
				{#each query.filenames as url, urlIdx}
					<a
						href={url}
						target="_blank"
						class="flex w-full items-center p-3 px-4 {urlIdx === query.filenames.length - 1
								? ''
...
<script lang="ts">
	...
	export let status = { urls: [], query: '', query_summaries: [] };
	let state = false;
</script>
