...
>
	...
	<div
		class="bg-lightGray-300 dark:bg-customGray-900 rounded-md relative"
	...
	>
		...
		<div class="flex flex-wrap gap-1 items-start p-3" on:click={() => inputRef.focus()}>
			{#each invitedEmails as email, index (email)}
				...
				>
					{email}
					<button type="button" class="ml-1 text-xs font-bold" on:click={() => removeEmail(email)}>
						×
					</button>
				</div>
			{/each}
	...
	<div class="mt-2.5 flex justify-end gap-7">
		...
		<button
			class=" text-xs w-[10rem] h-10 px-3 py-2 transition font-medium rounded-lg {loading
