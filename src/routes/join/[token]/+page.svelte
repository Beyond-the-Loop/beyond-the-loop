<script lang="ts">
	...
	const INPUT_CLASSES =
		'h-12 w-full rounded-lg border border-gray-200 bg-[#F1F1F1] px-4 text-sm text-[#16181D] outline-none transition focus:border-customBlue-500 focus:ring-1 focus:ring-customBlue-500 dark:border-customGray-700 dark:bg-customGray-800 dark:text-white';
	...
	onMount(() => {
		companyToken = $page.params.token ?? '';
		if (!companyToken) {
			goto('/auth');
		}
	});
	...
	const submit = async () => {
		...
		loading = true;
		const sessionUser = await signupWithCompanyToken(
			email.trim(),
			firstName,
			lastName,
			password,
		...
		const [companyInfo, companyConfigInfo] = await Promise.all([
			getCompanyDetails(sessionUser.token).catch(() => null),
			getCompanyConfig(sessionUser.token).catch(() => null)
		]);

		if (companyInfo) company.set(companyInfo);
		if (companyConfigInfo) companyConfig.set(companyConfigInfo);
...
<div class="flex min-h-screen bg-white dark:bg-customGray-900">
	<div class="flex w-full flex-col items-center px-6 sm:px-8 lg:w-1/2 lg:px-12 xl:px-16">
		...
		<div class="w-full max-w-[25rem] lg:max-w-[clamp(20rem,31.5vw,28rem)]">
			<input
				bind:this={profileImageInputRef}
				type="file"
				hidden
			...
			<form on:submit|preventDefault={submit}>
				...
				<div class="mb-4">
					<label for="join-email" class="mb-1.5 block text-sm font-normal text-[#16181D] dark:text-customGray-200">
						{$i18n.t('Work email')}
					</label>
					<input
						id="join-email"
						type="email"
						class="{INPUT_CLASSES} placeholder:text-gray-400 dark:placeholder:text-customGray-300{emailError ? ' border-red-500 focus:border-red-500 focus:ring-red-500' : ''}"
				...
				<div class="grid grid-cols-2 gap-3">
					<div>
						<label for="join-first-name" class="mb-1.5 block text-sm font-normal text-[#16181D] dark:text-customGray-200">
							{$i18n.t('First name')}
						</label>
						<input
							id="join-first-name"
					...
					<div>
						<label for="join-last-name" class="mb-1.5 block text-sm font-normal text-[#16181D] dark:text-customGray-200">
							{$i18n.t('Last name')}
						</label>
						<input
							id="join-last-name"
				...
				<div class="mt-4">
					<label for="join-password" class="mb-1.5 block text-sm font-normal text-[#16181D] dark:text-customGray-200">
						{$i18n.t('Password')}
					</label>
					<div class="relative">
						{#if showPassword}
				...
				<div class="mt-4">
					<label for="join-confirm-password" class="mb-1.5 block text-sm font-normal text-[#16181D] dark:text-customGray-200">
				...
				<p class="mt-6 text-center text-sm leading-relaxed text-[#6B7280] dark:text-customGray-300">
					...
					>
					{$i18n.t(
						'and that I am ordering as a business and not as a consumer within the meaning of § 13 BGB.'
					)}
				</p>
			</form>
		</div>
